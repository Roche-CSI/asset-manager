...
from asset_pluggy.storage import TransportResource
from asset_plugin_posix.transporter.async_posix_transporter import AsyncPosixTransporter
from asset_plugin_posix.transporter.posix_transport_resource import PosixUploadResource, PosixDownloadResource
...
def test_upload(test_data):
    ...
    for url_string in TEST_URLS:
        ...
        res = PosixUploadResource(src=os.path.join(test_data, url_string), dst=dst)
        targets.append(res)
...
def test_download(test_data):
    ...
    for url_string in TEST_URLS:
        ...
        res = PosixDownloadResource(src=os.path.join(test_data, url_string), dst=dst)
        targets.append(res)
...
def test_copy(test_data):
    ...
    for url_string in TEST_URLS:
        ...
        res = TransportResource(src=os.path.join(test_data, url_string), dst=dst)
        targets.append(res)
