...
class AwsHttpHandler:
    ...
    def get_storage_url(self, url_string: str, ignore: str = None) -> StorageURL:
        """Returns a BlobStoreURL instance.
    ...
    def fetch_blobs_list_from_bucket(self, bucket: str,
        ...
        aws_bucket = self.s3_resource.Bucket(bucket)
        # fetch blobs from the bucket filtered by prefix
        blobs = list(aws_bucket.objects.filter(Prefix=prefix))
