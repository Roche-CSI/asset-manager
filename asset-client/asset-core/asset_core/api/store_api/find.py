...
from asset_utils.utils.log_utils import LogColors, comma_formatted, kilo_byte
        ...
        try:
            if kwargs.get("alias"):
                ...
                if not server_access:
                    self.user_log.alert("skipping checking in asset-server since server access is disabled")
                    self.user_log.info("set 'server_access' config to 'true' to enable server access")
                    return None
