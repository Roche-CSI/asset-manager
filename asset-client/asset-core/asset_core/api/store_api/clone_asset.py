...
class CloneAssetAPI(StoreAPI):
    ...
    def remote_clone(self, asset_name: str, remote_url: str):
        ...
        self._download_asset(fetcher=fetcher, handle=handle)
    ...
    def __clone_individual_asset(self, name,
        ...
        with ch_dir(asset_dst):
            ...
            if soft:
                ...
                return {handle.asset_name: asset_dst}
    ...
    def _download_asset(self, fetcher: AssetFetcher, handle: AssetHandle):
        """Downloads the asset metadata at the asset store"""
        asset_url = self.store.asset_url(handle.class_id, handle.seq_id)
        if not fetcher.download_dir(dir_url=asset_url,
                                    dir_dst=self.store.asset_cache(class_id=handle.class_id, seq_id=handle.seq_id),
                                    progress=f"fetching asset information from remote ({handle.asset_name})"):
            self.user_log.message("asset data exists - skipping download")
