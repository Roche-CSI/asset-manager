...
class ListAPI(StoreAPI):
    ...
    def list_assets(self, class_name):
        if not class_name:
        ...
        try:
            ...
            assets: dict = asset_class.list_assets()
            if not assets:
            ...
            self.print_assets_list_table(class_name=class_name, assets=self._update_locations(assets=assets))
            cmd = UserCommands()
            self.user_log.message("\n".join([cmd.clone_asset(), cmd.fetch_assets(), cmd.list_help()]))
        except exceptions.AssetClassNotFoundError as e:
            self.user_log.message(body=e.msg, color=LogColors.ALERT)
    ...
    def print_assets_list_table(self, class_name: str, assets: list):
        if not assets:
            message = colored_string(f"there are no assets locally for class: {class_name} \n",
                                     LogColors.INFO)
            message += f"to fetch the list of assets from remote: {UserCommands().fetch_assets()} \n"
            message += f"to view the the list of classes: {UserCommands().list_classes()}"
            self.user_log.message(message)
            return
        ...
        data = [{
            ...
            "refs": self.active_asset_color(obj.get("id"), location=obj.get("location"), string=obj.get("refs")),
            ...
            # for idx, obj in enumerate(sorted(assets.values(), key=lambda x: self.parse_seq_id(x.get("seq_id"))))
        ]
