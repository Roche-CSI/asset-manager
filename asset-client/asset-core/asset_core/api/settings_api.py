...
class SettingsAPI(LoggingMixin):
    ...
    def _login_with_email(self, jsonize=False):
        # TODO: update to google_oauth_v2() after server is updated
    ...
    def user_signup(self, username: str, email: str):
        with self.user_settings():
            ...
            if not self.valid_email(email=email):
                raise exceptions.AssetException(msg="invalid email, you can only signup with a valid roche email")
    ...
    def save_user(self, res: dict, jsonize=False):
        ...
        if user and user.get("id"):
            ...
            if self.settings.active_project:
                ...
                e.logs.add("To create a new project or get access: DASHBOARD_URL",
                           LogColors.INFO)
                raise e
        else:
    ...
    def set_asset_home(self, dst_dir):
        ...
        if not self.validate_dst(AssetStore.get_store_dir(dir_path=dst_dir)):
            return

        # CREATE NEW ASSET STORE
        # if there is an existing store, ask user should we move the store to the new location or
        # delete the old one and create afresh
        existing_store = self.store_exists()
        user_input = None
        if existing_store:
            msg = f"found an existing store at:{existing_store.home_dir}, please choose:\n"
            msg += f"1 move existing store to new location:{dst_dir}\n"
            msg += "2 delete existing store and create a new one\n"
            msg += "3 keep the existing store and create a new one\n"
        ...
        if not user_input:
            ...
            # move store to new location
            FileUtils.move(existing_store.home_dir, dst_dir)
            self.settings.assets_home = dst_dir
        elif user_input == "2":
            # create new and then delete the old one
            existing_store_assets = existing_store.home_dir
            self.settings.assets_home = dst_dir
            AssetStore.create_store()
            # delete existing
            shutil.rmtree(existing_store_assets)
        elif user_input == "3":
            # existing_store_assets = existing_store.project_dir
            self.settings.assets_home = dst_dir
            AssetStore.create_store()
        else:
            raise exceptions.AssetException("invalid option for setting asset-store")

        self.user_log.success("Success")
        self.user_log.info(f"asset-home is now set to: {dst_dir}")

    def validate_dst(self, dst_dir):
        try:
            AssetStore.validate_store_dir(dst_dir)
            return True
        except exceptions.AssetStoreCreateError as e:
            self.user_log.message(str(e), LogColors.ERROR)
            return False
