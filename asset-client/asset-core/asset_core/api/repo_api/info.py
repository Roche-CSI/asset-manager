...
class InfoAPI(RepoAPI):
    ...
    def asset_info(self, large=False, jsonize=False):
        if not self.asset.repo.current_asset:
            message = f"{colored_string('There are no active assets', color=LogColors.ERROR)} \n"
            message += f"{UserCommands().clone_asset()} \n"
            message += f"{UserCommands().create_asset()} \n"
            message += f"{UserCommands().list_assets()} \n"
    ...
    def print_asset(self, jsonize=False):
        ...
        rows = self.asset_summary(asset=self.asset, project=project)
        if jsonize:
            return rows
    ...
    def asset_summary(self, asset, project) -> dict:
        """Returns a summary info of the asset"""
        summary = {
            "project": project.get("name"),
            "asset": asset.name,
            "version": asset.version.number or "None",
            "size": f"{comma_formatted(kilo_byte(asset.objects.size))} KB",
            "hash": asset.objects.hash,
            "created_by": asset.created_by,
            "created_at": asset.created_at,
            "asset id": asset.id,
            "asset class": f"{asset.asset_class.name}",
            "alias": self.asset.alias or 'None',
            "refs": [str(ref) for ref in self.asset.refs],
    ...
    def print_objects(self, objects=None,
                      base_dir=os.getcwd(),
                      large=False,
                      jsonize=False):
