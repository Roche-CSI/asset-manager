...
class InitAssetAPI(RepoAPI):
    ...
    def create_asset(self, class_name: str, location: str = None):
        ...
        self.user_log.message(self.docs_message())
        return repo.fs_path
    ...
    def create_repo(self, location: str = None) -> (Repo, bool):
        ...
        try:
            ...
            if os.path.samefile(repo_dir, home_dir):
    ...
    def docs_message(self):
        docs_url = Configs.shared().docs_url
        help_url = Configs.shared().DOCS.get("ISSUE")
        github_url = Configs.shared().DOCS.get("GITHUB")
        return f"""Check out the following:
                - documentation: {format_link(docs_url)}
                - get help and share ideas: {format_link(help_url)}
