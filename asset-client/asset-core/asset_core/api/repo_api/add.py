...
class AddAPI(RepoAPI):
    ...
    def validate_alias(self, alias: str) -> bool:
        if not alias:
            self.user_log.error("error: missing alias")
            return False

        # must not be an integer
        if is_integer(alias):
            self.user_log.error("error: alias must be a string")
            return False
        ...
        if contains_special_chars(alias):
            self.user_log.error("error: alias cannot contain any special characters other than '_', '.', '-'")
            return False

        # validate that it doesn't match with temp_seq_id
        if Asset.is_temp_seq_id(seq_id=alias):
            self.user_log.error(f"error: alias cannot start with: {Asset.TEMP_SEQ_PREFIX}")
            return False
