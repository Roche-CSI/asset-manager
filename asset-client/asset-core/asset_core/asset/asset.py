...
class Asset(SerializableAsset):
    ...
    def db(self) -> ManifestDB | None:
        if self.manifest_file:
            return ManifestDB(path=self.manifest_file, backup_path=self.manifest_backup_ile)
        return None

    @cached_property
    def object_stats_db(self) -> FileDB:
        return FileDB(path=self.repo.objects_stats_file, backup_path=self.repo.objects_stats_backup_file)
    ...
    def states_db(self) -> StatesDB:
        return StatesDB(path=self.states_file, backup_path=self.states_backup_file)
    ...
    @property
    def states_backup_file(self):
        if not self.id:
            return None
        return self.__class__.states_backup_path(repo=self.repo, asset_id=self.id, version=self.version.number)
    ...
    @property
    def manifest_backup_ile(self):
        if not self.id or not self.repo or not self.version.number:
            return None
        return self.__class__.manifest_backup_path(repo=self.repo, asset_id=self.id, version=self.version.number)
    ...
    @classmethod
    def manifest_backup_path(cls, repo, asset_id, version):
        return os.path.join(repo.manifests_backup_dir, asset_id, f"{version}.json")
    ...
    @classmethod
    def states_backup_path(cls, repo, asset_id, version):
        return os.path.join(repo.states_backup_dir, asset_id, f"{version}.json")
