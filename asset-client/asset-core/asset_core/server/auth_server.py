...
from google_auth_oauthlib.flow import _RedirectWSGIApp, InstalledAppFlow
...
class AuthServer(BaseServer):
    ...
    def _auth_route(self):
        return os.path.join(self.url, self.configs.auth_route)
    ...
    def _login_route(self):
        return os.path.join(self.url, self.configs.email_login_route)
    ...
    def get_auth(self):
        return self.parse(self.get(url=self._auth_route()))
    ...
    def login_with_google(self, data: dict) -> dict:
    ...
    def google_oauth(self):
        # fetch client credentials from server
        # If there are no (valid) credentials available, let the user log in.
        configs = self.get_auth()
        flow = InstalledAppFlow.from_client_config(client_config=configs,
                                                   scopes=configs.get("web").get("scopes"))
        creds = flow.run_local_server(port=0)
        # Save the credentials for the next run
        res = self.login_with_google(data={
            "client_id": creds.client_id,
            "id_token": creds.id_token
        })
        return res

    def google_oauth_v2(self):
